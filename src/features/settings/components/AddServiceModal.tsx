...
import { createService } from '../settings.api';
import type {
  ServiceFormValues,
  ServiceInsert,
} from '../settings.types';
...
function getServiceErrorMessage(error: unknown): string {
  ...
  if (
    ...
    return 'אין לך הרשאה להוסיף שירותים לעסק.';
  }

  return 'לא ניתן להוסיף את השירות. בדוק את הפרטים ונסה שוב.';
}
...
export default function AddServiceModal({
  ...
  const [formData, setFormData] =
    useState<ServiceFormValues>(INITIAL_VALUES);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error('שגיאה בהוספת שירות:', getErrorMessage(error));
  ...
  return (
    ...
    >
      ...
      >
        <h2 id="add-service-title" className={styles.title}>
          הוספת שירות חדש
        </h2>
